...
ALTER TABLE orders ADD CONSTRAINT orders_payment_method_check
  CHECK (payment_method IN ('bank_transfer', 'easypaisa', 'cash', 'pay_on_gate'));

ALTER TABLE orders ADD COLUMN IF NOT EXISTS paid BOOLEAN DEFAULT true;
...
AS $$
  SELECT jsonb_build_object(
    ...
    'online_sales_count', COUNT(*) FILTER (WHERE payment_method NOT IN ('cash', 'pay_on_gate') AND payment_status = 'approved')::int,
    ...
    'online_revenue', COALESCE(SUM(total_amount) FILTER (WHERE payment_method NOT IN ('cash', 'pay_on_gate') AND payment_status = 'approved'), 0)
  ) FROM orders;
$$;
